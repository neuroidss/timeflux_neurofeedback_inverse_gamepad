...
class InputDevice(Node):
    ...
    def update(self):
        ...
        if not sys.platform.startswith("win"):
            ...
            now = clock.now()
            indices.append(now)
            for etype, ecodes in _input.ioctl_capabilities(
                self._evdev_device.fd
            ).items():
                for code in ecodes:
                ...
                for name, port in self.ports.items():
                    ...
                    if port.data is not None:
                        for (colname, colval) in port.data.items():
                            ...
                            if not np.isnan(val):
                                if colname == "XR":
                                    self._vjoy_device.data.wAxisXRot = round(
                                        0x8000 * val
                                    )
                                if colname == "YR":
                                    self._vjoy_device.data.wAxisYRot = round(
                                        0x8000 * val
                                    )
                                if colname == "ZR":
                                    self._vjoy_device.data.wAxisZRot = round(
                                        0x8000 * val
                                    )
                                if colname == "X":
                    ...
                    if port.data is not None:
                        ...
                        for (colname, colval) in port.data.items():
                            ...
                            if not np.isnan(val):
                                ...
                                if colname == "XR":
                                    self._uinput_device.emit(
                                        uinput.ABS_RX, round(0x8000 * val)
                                    )
                                if colname == "YR":
                                    self._uinput_device.emit(
                                        uinput.ABS_RY, round(0x8000 * val)
                                    )
                                if colname == "ZR":
                                    self._uinput_device.emit(
                                        uinput.ABS_RZ, round(0x8000 * val)
                                    )
                                if colname == "X":
                                    self._uinput_device.emit(
                                        uinput.ABS_X, round(0x8000 * val)
                                    )
                                if colname == "Y":
                                    self._uinput_device.emit(
                                        uinput.ABS_Y, round(0x8000 * val)
                                    )
                                if colname == "Z":
                                    self._uinput_device.emit(
                                        uinput.ABS_Z, round(0x8000 * val)
                                    )
                                if (
                                    (colname.find("B") == 0)
                                    and (int(colname[1:]) >= 1)
                                    and (int(colname[1:]) <= 17)
                                ):
                                    if round(val) > 0:
                                        self._uinput_device.emit(
                                            (
                                                uinput.BTN_GAMEPAD[0],
                                                uinput.BTN_GAMEPAD[1]
                                                + (int(colname[1:]) - 1),
                                            ),
                                            1,
                                        )
