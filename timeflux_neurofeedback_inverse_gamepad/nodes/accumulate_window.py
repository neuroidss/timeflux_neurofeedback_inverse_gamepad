...
class Window(Node):
    ...
    def __init__(self, length, step=None, index="time", epochs=False, accumulate=False):
        ...
        self.__init__(length, step, accumulate)
        if epochs:
            self.bind("o", "o_0")
...
class TimeWindow(Node):
    def __init__(self, length, step=None, accumulate=False):
        if step is None:
            step = length
        if step > length:
