...
class StableDiffusion(Node):
  ...
  def __init__(self, to_screen=False, huggingface_hub_token='huggingface_hub_token', unet_height=512, unet_width=512, num_inference_steps=10, unet_num_inference_steps=10, unet_latents=None, unet_guidance_scale=7.5, apply_to_latents=10, apply_to_embeds=10, clip_prompt='villa by the sea in florence on a sunny day', output_path=''):
   """
        Args:
...
#        self._value = value
   ...
   if True:
    ...
    self._vae = AutoencoderKL.from_pretrained(
        'CompVis/stable-diffusion-v1-4', subfolder='vae', use_auth_token=self._huggingface_hub_token
    )
    ...
    self._unet = UNet2DConditionModel.from_pretrained(
        'CompVis/stable-diffusion-v1-4', subfolder='unet', use_auth_token=self._huggingface_hub_token
    )
   ...
   if True:
    ...
    def get_text_embeds(prompt):
        ...
        with torch.no_grad():
            text_embeddings = text_encoder(text_input.input_ids.to(self._device))[0]
        ...
        with torch.no_grad():
            uncond_embeddings = text_encoder(uncond_input.input_ids.to(self._device))[0]
                      ...
                      if port.data is not None:
                        for (colname,colval) in port.data.items():
                          ...
                          if cons_index < len(cons_latents_flatten):
                            cons_latents_flatten[cons_index] = ((val+self._apply_to_latents)/1+0.0001)/((1+self._apply_to_latents)/1+0.0001)
                      ...
                      if port.data is not None:
                        for (colname,colval) in port.data.items():
                          ...
                          if cons_index < len(cons_embeds_flatten):
                            cons_embeds_flatten[cons_index] = ((val+self._apply_to_embeds)/1+0.0001)/((1+self._apply_to_embeds)/1+0.0001)
                            cons_index = cons_index + 1
...
#                      print(i1,t1,sigma1)
                      ...
                      with torch.no_grad():
                          noise_pred = self._unet(latent_model_input, t1, encoder_hidden_states=text_embeddings)['sample']
            
                      noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                      noise_pred = noise_pred_uncond + guidance_scale1 * (noise_pred_text - noise_pred_uncond)

                      latents = self._scheduler.step(noise_pred, i1, latents)['prev_sample']
...
#            if True:    
            if False:
              ...
              latents = self._scheduler.step(noise_pred, i_tqdm, latents)['prev_sample']
...
#                  latents=cons_latents,
            ...
            #print(images[0])
            images[0].save(self._output_path+'stable-diffusion-'+self._dt_string+'-'+self._clip_prompt+'.png', format='png')
            images[0].save('mygraph.png', format='png')
