...
class Replay(Node):
    ...
    def __init__(self, filename, keys=None, speed=1, timespan=None, resync=True, start=0):
        """
        Initialize.
...
#            self._store = pd.HDFStore('store.h5')
            self._bdf = mne.io.read_raw_bdf(self._find_path(filename), eog=None, misc=None,
...
#                print(self._store.pick_channels([key]))
                first = self._meas_date + pd.Timedelta(value=self._bdf[0][1][0], unit='seconds')
...
#                last = self._store.select(key, start=nrows - 1, stop=nrows).index[0]
                last = self._meas_date + pd.Timedelta(value=self._bdf[0][1][nrows - 1], unit='seconds')
...
#            data = self._store.select(key, "index >= min & index < max")
            data = self._bdf.get_data([key], tmin=pd.Timedelta(min-self._meas_date).seconds, tmax=pd.Timedelta(max-self._meas_date).seconds)
            data = data * 1000000
...
class Save(Node):
    ...
    def __init__(
        ...
        sample_rate=None, eeg_channels=None, pmax=312500, pmin=-312500, dmax=8388607, dmin=-8388608,
        dimension="uV", data_key="eeg", file_type=pyedflib.FILETYPE_BDFPLUS
    ):
        """
        Initialize.
 ...
 #               path, time.strftime("%Y%m%d-%H%M%S.hdf5", time.gmtime())
                path, time.strftime("%Y%m%d-%H%M%S.bdf", now)
            )
        else:
...
#        file_type = pyedflib.FILETYPE_BDFPLUS  # BDF+
        self._bdf = pyedflib.EdfWriter(filename, n_channels=n_channels, file_type=file_type)
